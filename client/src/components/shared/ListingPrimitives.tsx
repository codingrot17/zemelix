...
export function BadgeChip({ badge, className }: { badge: BadgeVariant; className?: string }) {
    const style = BADGE_STYLES[badge] ?? "bg-gray-500 text-white";
    return (
        <span className={cn("inline-block px-2.5 py-0.5 rounded-full text-xs font-bold tracking-wide shadow-sm", style, className)}>
            {badge}
        </span>
    );
}

export function ListingTypeChip({ vendorType, size = "sm" }: { vendorType: VendorType | string; size?: "xs" | "sm" }) {
    ...
    return (
        <span className={cn(
            ...
            {isService ? <Clock className="w-3 h-3" /> : <Package className="w-3 h-3" />}
...
export function SellerStrip({ name, avatar, vendorType, className }: {
    name: string;
    avatar?: string | null;
    ...
    return (
        <div className={cn("flex items-center gap-2", className)}>
            {avatar ? (
                <img src={avatar} alt={name} className="w-6 h-6 rounded-full border object-cover flex-shrink-0" />
            ) : (
...
export function ListingActionButton({ vendorType, outOfStock, added, onClick, size = "sm", className }: {
    vendorType: VendorType | string;
    outOfStock?: boolean;
    added?: boolean;
    ...
    const isService = vendorType === "service";
    const base = "inline-flex items-center justify-center gap-1.5 rounded-lg font-semibold transition focus:outline-none focus:ring-2";
    ...
    return (
        <button className={cn(base, sizeClass, colorClass, className)} onClick={onClick} disabled={outOfStock} type="button">
            {added ? "Added ✓" : outOfStock ? "Out of Stock" : isService ? <><Bookmark className="w-3.5 h-3.5" />Book Now</> : <><ShoppingCart className="w-3.5 h-3.5" />Add to Cart</>}
        </button>
    );
}
