...
interface ProductForm { title: string; shortDescription: string; longDescription: string; price: string; stock: string; category: string; tags: string; badge: string; vendorType: string; sellerWhatsapp: string; }
const EMPTY_FORM: ProductForm = { title: "", shortDescription: "", longDescription: "", price: "", stock: "", category: "", tags: "", badge: "", vendorType: "seller", sellerWhatsapp: "" };
...
export default function SellerProducts() {
    ...
    const openEdit = (product: SellerProduct) => {
        ...
        setForm({ title: product.title, shortDescription: product.shortDescription, longDescription: product.longDescription ?? "", price: String(product.price), stock: String(product.stock), category: product.category, tags: product.tags?.join(", ") ?? "", badge: product.badge ?? "", vendorType: product.vendorType, sellerWhatsapp: product.sellerWhatsapp ?? "" });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const payload = { title: form.title.trim(), shortDescription: form.shortDescription.trim(), longDescription: form.longDescription.trim() || null, price: Number(form.price), stock: Number(form.stock), category: form.category, tags: parseTags(form.tags), badge: form.badge || null, vendorType: form.vendorType, sellerWhatsapp: form.sellerWhatsapp.trim() || null, imageUrl: imageUrl || null };
    ...
    return <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="Search by title or category…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9" /></div></div>
